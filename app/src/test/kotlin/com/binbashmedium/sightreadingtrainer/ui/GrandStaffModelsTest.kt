...
class GrandStaffModelsTest {
    ...
    fun `formatChordLabelShort omits roman numeral and stays compact`() {
        ...
        // Suspended voicing (D-G-A) is harmonically ambiguous (Gsus2 or Dsus4); either is acceptable.
        assertTrue(formatChordLabelShort(listOf(62, 67, 69)) in setOf("Gsus2", "Dsus4"))
        // Seventh chord
    ...
    fun `resolveDisplayChordNotes resolves suspended chord runs`() {
        ...
        val labels = resolved.map { formatChordLabelShort(it.orEmpty()) }.toSet()
        assertTrue(labels.size == 1)
        assertTrue(labels.first() in setOf("Gsus2", "Dsus4"))
    }
